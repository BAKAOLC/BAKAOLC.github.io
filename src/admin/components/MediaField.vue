...
<template>
  ...
  >
    ...
    <div v-if="!field.multiple" class="single-media-control" :class="{ 'has-value': values.length }">
      <button type="button" class="single-media-preview" :title="values.length ? t('media.replaceFromManager') : t('media.chooseFromManager')" @click="openLibrary(values.length ? 0 : undefined)">
        <img v-if="isImage && values.length" :src="resolveAssetUrl(values[0])" :alt="field.label" loading="lazy" decoding="async">
    ...
    <div v-else-if="values.length" ref="grid" class="media-card-grid">
      <article v-for="(path, index) in values" :key="`${path}-${index}`" class="media-card">
        <div class="media-card-preview">
          <img v-if="isImage" :src="resolveAssetUrl(path)" :alt="field.label" loading="lazy" decoding="async">
    ...
    <button v-else type="button" class="media-empty-drop" :disabled="uploading" @click="input?.click()">
